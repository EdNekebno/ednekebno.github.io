...
typedef struct riv_context {
  ...
  //////////////////////////////////////
  // Buffers
  // This is a problem - having the last 4 here crashes when running as a cartridge
  // We kinda need the last one but maybe we can bit bash it out of memory
...
riv_context * riv_get_context(void);
...
bool riv_is_key_repeat_press(uint8_t key, uint64_t repeat_delay, uint64_t repeat_rate);
...
riv_recti riv_get_image_bbox(uint64_t img_id, int64_t sx0, int64_t sy0, int64_t w, int64_t h);
// Get bounding box from a sprite
riv_recti riv_get_sprite_bbox(uint32_t n, uint64_t sps_id, int64_t nx, int64_t ny);
...
riv_recti riv_draw_text_ex(const char* text, uint64_t sps_id, riv_align anchor, int64_t x, int64_t y, int64_t mw, int64_t mh, int64_t sx, int64_t sy, int64_t col);
...
//uint64_t riv_mempeek(uint64_t addr, uint64_t size);
// Write up to 8 bytes to memory address
//void riv_mempoke(uint64_t addr, uint64_t value, uint64_t size);
// Fill memory region with a constant byte
//void riv_memset(uint64_t addr, uint8_t value, uint64_t size);
// Copy bytes from memory a region source to another memory region, the region may overlap
//void riv_memmove(uint64_t dest_addr, uint64_t src_addr, uint64_t size);
// Scan memory region for a byte and return the address for the matching byte
//uint64_t riv_memscan(uint64_t addr, uint8_t value, uint64_t size);
// Compare memory areas, returns zero if they are equal
//int32_t riv_memcmp(uint64_t addr1, uint64_t addr2, uint64_t size);
// Allocate memory region and return its starting address
uint64_t riv_memalloc(uint64_t size);
// Reallocate memory region to a new size
//uint64_t riv_memrealloc(uint64_t addr, uint64_t new_size, uint64_t old_size);
// Deallocate memory from starting address
//void riv_memdealloc(uint64_t addr);
...
function new_image(width, height, colorkey, data)
    c = 1
    for u = 1, L.RIV_MAX_IMAGES - 1 do
        if ffi.string(L.riv.images[u].pixels) == nil then
            break
        end
        c = c + 1
    end
    L.riv.images[c].width = width
    L.riv.images[c].height = height
    L.riv.images[c].color_key = colorkey
    L.riv.images[c].owned = true
    L.riv.images[c].pixels = ffi.C.malloc(string.len(data) / 2)
    pc = 0
    for u = 0, (string.len(data) / 2) - 1 do
        v = u * 2
        v = v + 1
        tc = string.sub(data, v, v) .. string.sub(data, (v + 1), v + 1)
        L.riv.images[c].pixels[pc] = tonumber(tc, 16)
        pc = pc + 1
    end
    nl = 0
    lwtff = 0
    ...
    for u = 0, (pc - 1) do
        if ignore == 0 then
            if L.riv.images[c].pixels[u] ~= 255 then
                nl = nl + 1
            else
                nl = nl + L.riv.images[c].pixels[u + 1]
                ignore = 2
            end
        else
            ignore = ignore - 1
    ...
    for u = 0, (pc - 1) do
        if ignore == 0 then
            if L.riv.images[c].pixels[u] ~= 255 then
                nl = nl + 1
                ni[o] = L.riv.images[c].pixels[u]
                o = o + 1
            else
                nl = nl + L.riv.images[c].pixels[u + 1]
                cnt = L.riv.images[c].pixels[u + 1]
                for pp = 0, cnt - 1 do
                    ni[o] = L.riv.images[c].pixels[u + 2]
                    o = o + 1
                end
                ignore = 2
            end
    ...
    ffi.C.free(L.riv.images[c].pixels)
    L.riv.images[c].pixels = ffi.C.malloc(nl)
    ffi.copy(L.riv.images[c].pixels, ni, nl)
    return c
end
